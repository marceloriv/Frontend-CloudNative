  ...
  const colorClass =
    estado === 'confirmada'
      ...
      : estado === 'pendiente'
        ? 'bg-alert-yellow'
        : 'bg-gray-300'
  return <div className={`w-1 self-stretch rounded-l-xl flex-shrink-0 ${colorClass}`} aria-hidden="true" />
}
      ...
      <div className="flex flex-1 gap-4 p-5">
        ...
        <div className="flex flex-1 flex-col gap-2 min-w-0">
          <div className="flex flex-wrap items-start gap-2">
            ...
            <span className="px-2 py-0.5 bg-gray-100 text-muted text-xs rounded-full">{reserva.categoria}</span>
          </div>
          <p className="text-sm text-muted">
            {reserva.fechaDisplay} · {reserva.hora} – {endTime} hrs
          </p>
          <div className="flex flex-wrap items-center gap-3 mt-1">
            <span className="font-medium text-text">{formatPrecio(reserva.precio * reserva.duracionHrs)}</span>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            >
              {ESPACIOS.map((esp, idx) => (
                <option key={esp.nombre} value={idx}>
                  {esp.nombre} — {esp.tarifaHr === 0 ? 'Gratis' : `$${esp.tarifaHr.toLocaleString('es-CL')}/hr`}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex flex-col gap-1">
              ...
              >
                {DURACIONES.map((d) => (
                  <option key={d} value={d}>{d} {d === 1 ? 'hr' : 'hrs'}</option>
                ))}
              </select>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-lg p-3 text-sm text-text">
            ...
            {form.duracion} {form.duracion === 1 ? 'hr' : 'hrs'} &times; {espacio.tarifaHr === 0 ? 'Gratis' : `$${espacio.tarifaHr.toLocaleString('es-CL')}`} ={' '}
            <span className="font-semibold text-primary">{total === 0 ? 'Gratis' : `$${total.toLocaleString('es-CL')}`}</span>
          </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
        ...
        <p className="text-sm text-muted mb-1">
          <span className="font-medium text-text">{reserva.espacio}</span> — {reserva.fechaDisplay} a las {reserva.hora} hrs
        </p>
        <p className="text-sm bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-text mt-3 mb-6">
          ⚠️ Recuerda que las cancelaciones deben realizarse con al menos 24 horas de anticipación. Esta acción no se puede deshacer.
        </p>
...
export default function Reservas() {
  ...
  const filtered = activeTab === 'todas' ? reservas : reservas.filter((r) => r.estado === activeTab)
  const cancelTargetReserva = cancelTarget ? reservas.find((r) => r.id === cancelTarget) ?? null : null
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-text px-6 py-8">
        <div className="max-w-3xl mx-auto flex items-start justify-between gap-4">
          <div>
            <h1 className="font-display text-3xl text-white">{isAdmin ? 'Reservas del condominio' : 'Mis Reservas'}</h1>
            <p className="text-white/60 mt-1">{isAdmin ? 'Gestión y bloqueo por mantenimiento' : 'Torres del Parque'}</p>
          </div>
          {!isAdmin && (
            <button
      ...
      {blockTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/50" onClick={() => setBlockTarget(null)} aria-hidden="true" />
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
            <h2 className="font-display text-xl text-text mb-2">¿Bloquear por mantenimiento?</h2>
            <p className="text-sm text-muted mb-6">
              Esta reserva quedará cancelada y el espacio aparecerá no disponible en el período correspondiente. La acción no se puede deshacer.
            </p>
            <div className="flex gap-3">
              <button
