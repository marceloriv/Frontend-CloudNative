...
const categorias = ['Todos', 'Jardinería', 'Pintura', 'Mantención eléctrica', 'Limpieza de fachada']
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{
      ...
      <div style={{ position: 'relative', height: 200, background: '#E2E8F0', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', top: 12, right: 12, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', color: '#fff', fontSize: 11, fontWeight: 500, padding: '4px 10px', borderRadius: 100 }}>
          {registro.categoria}
        </div>
      </div>
      ...
      <div style={{ padding: '18px 18px 20px' }}>
        ...
        <h3 style={{ fontFamily: 'Gloock, Georgia, serif', fontSize: 16, color: '#00201B', margin: '0 0 6px', fontWeight: 400, lineHeight: 1.3 }}>{registro.titulo}</h3>
        <div style={{ fontSize: 13, color: '#64748B', marginBottom: 4 }}>{registro.responsable}</div>
        <div style={{ fontSize: 14, fontWeight: 700, color: '#00201B', marginBottom: 14 }}>{registro.monto}</div>

        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#0D9488', fontWeight: 500 }}>
            <IconCheck style={{ width: 12, height: 12 }} /> {registro.boleta}
          </div>
          <button onClick={onOpen} style={{
            display: 'flex', alignItems: 'center', gap: 6, background: '#F0FDFA', border: '1px solid #CCFBF1',
            color: '#005047', borderRadius: 8, padding: '8px 14px', fontSize: 13, fontWeight: 600, cursor: 'pointer',
    ...
    <div style={{ minHeight: '100vh', background: '#F8FAFB' }}>
      ...
      <div style={{ background: '#00201B', padding: '72px 24px 64px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.45)', letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 16 }}>Bitácora fotográfica</p>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0D9488 0%, #005047 100%)', padding: '40px 24px 36px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.6)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 10 }}>Bitácora fotográfica</p>
          <h2 style={{ fontFamily: 'Gloock, Georgia, serif', fontSize: 'clamp(24px, 3vw, 36px)', color: '#fff', lineHeight: 1.1, margin: '0 0 10px', fontWeight: 400 }}>Registro Fotográfico</h2>
          <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.72)', margin: 0, maxWidth: 560, lineHeight: 1.65 }}>
            Control de calidad verificable por todos los residentes. Cada intervención documentada con fotos de antes y después, fecha, responsable y boleta adjunta.
          </p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '40px 24px 80px' }}>
        ...
        <div style={{ display: 'flex', gap: 16, marginBottom: 28, flexWrap: 'wrap' }}>
          {[
            { val: registroFotos.length, label: 'Trabajos registrados', color: '#0D9488' },
            { val: registroFotos.filter(r => r.estado === 'Completado').length, label: 'Completados', color: '#005047' },
            { val: '100%', label: 'Con boleta adjunta', color: '#14B8A6' },
          ].map(s => (
            <div key={s.label} style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, padding: '16px 22px', display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontFamily: 'Gloock, Georgia, serif', fontSize: 28, color: s.color }}>{s.val}</span>
              <span style={{ fontSize: 13, color: '#64748B', maxWidth: 100, lineHeight: 1.3 }}>{s.label}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 22 }}>
          {filtered.map(r => (
            <RegistroCard key={r.id} registro={r} onOpen={() => { setSelected(r); setShowBefore(true) }} />
          ))}
        </div>
      </div>
          ...
          <div style={{ position: 'relative', background: '#fff', borderRadius: 20, width: '100%', maxWidth: 680, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 32px 80px rgba(0,0,0,0.25)' }}>
            <div style={{ position: 'relative', height: 320, background: '#E2E8F0', overflow: 'hidden', borderRadius: '20px 20px 0 0' }}>
              ...
              <div style={{ position: 'absolute', bottom: 16, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 8 }}>
                {['Antes', 'Después'].map((label, i) => (
                  <button key={label} onClick={() => setShowBefore(i === 0)} style={{
            ...
            <div style={{ padding: '28px 28px 32px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 16 }}>
                <div>
                  <span style={{ fontSize: 11, fontWeight: 700, color: '#005047', background: '#CCFBF1', padding: '3px 10px', borderRadius: 6, display: 'inline-block', marginBottom: 8 }}>{selected.categoria}</span>
                  <h2 style={{ fontFamily: 'Gloock, Georgia, serif', fontSize: 22, color: '#00201B', margin: 0, fontWeight: 400, lineHeight: 1.25 }}>{selected.titulo}</h2>
                </div>
                <span style={{ fontSize: 11, color: '#94A3B8', whiteSpace: 'nowrap', marginTop: 4 }}>{selected.id}</span>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
                {[
                  ...
                  <div key={f.label}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>{f.label}</div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: '#00201B' }}>{f.val}</div>
                  </div>
                ))}
              </div>

              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#F0FDFA', borderRadius: 10, padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#005047', fontWeight: 600 }}>
                  <IconCheck style={{ width: 16, height: 16 }} /> Boleta adjunta: {selected.boleta}
                </div>
                <button style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', color: '#0D9488', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
