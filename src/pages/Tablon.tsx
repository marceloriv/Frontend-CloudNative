...
import {
  IconPlus, IconBell, IconCheck, IconUsers,
  IconCalendar, IconMessage, IconHome, IconChevronRight,
} from '../components/icons/Icons'

const tipos = ['Todos', 'Asamblea', 'Mantención', 'Aviso']

type BadgeKey = 'primary' | 'accent' | 'warning'
const badgeClasses: Record<BadgeKey, { border: string; label: string; text: string }> = {
  primary: { border: 'border-l-primary',      label: 'bg-primary/10 text-primary',      text: 'text-primary' },
  accent:  { border: 'border-l-accent',       label: 'bg-accent/10 text-accent',        text: 'text-accent' },
  warning: { border: 'border-l-alert-yellow', label: 'bg-alert-yellow/20 text-text',    text: 'text-text' },
}
    ...
    <div style={{ minHeight: '100vh', background: '#F8FAFB' }}>
      ...
      <div style={{ background: '#00201B', padding: '72px 24px 64px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.45)', letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 16 }}>Cartelera digital</p>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0D9488 0%, #005047 100%)', padding: '40px 24px 36px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 20 }}>
          <div>
            <p style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.6)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 10 }}>Cartelera digital</p>
            <h2 style={{ fontFamily: 'Gloock, Georgia, serif', fontSize: 'clamp(24px, 3vw, 36px)', color: '#fff', lineHeight: 1.1, margin: '0 0 10px', fontWeight: 400 }}>Tablón de Eventos y Avisos</h2>
            <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.72)', margin: 0, maxWidth: 520, lineHeight: 1.65 }}>
              Cartelera digital comunitaria con avisos, asambleas y comunicados del comité — en tiempo real.
            </p>
          </div>
          <button onClick={() => setShowNew(true)} style={{
            display: 'flex', alignItems: 'center', gap: 8, background: '#fff', color: '#005047',
            border: 'none', borderRadius: 10, padding: '12px 20px', fontSize: 14, fontWeight: 700,
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '40px 24px 80px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16, marginBottom: 32 }}>
          <div style={{ display: 'flex', gap: 8 }}>
            {tipos.map(t => (
              <button key={t} onClick={() => setActiveTipo(t)} style={{
            ...
            return (
              <div key={i} className={`bg-surface border border-border border-l-4 ${bc.border} rounded-2xl overflow-hidden transition-[box-shadow,transform] duration-200 hover:shadow-lg hover:-translate-y-0.5`}>
                <div style={{ padding: '22px 22px 0' }}>
                  ...
                  <h3 style={{ fontFamily: 'Gloock, Georgia, serif', fontSize: 17, color: '#00201B', margin: '0 0 10px', fontWeight: 400, lineHeight: 1.3 }}>{a.titulo}</h3>
                  <p style={{ fontSize: 13, color: '#64748B', lineHeight: 1.65, margin: '0 0 16px' }}>{a.desc}</p>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#94A3B8', marginBottom: 16 }}>
                    <IconBell style={{ width: 12, height: 12 }} /> Publicado por: <strong style={{ color: '#64748B' }}>{a.autor}</strong>
                  </div>
                </div>
                {a.confirmacion && (
                  <div style={{ borderTop: '1px solid #F1F5F9', padding: '14px 22px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <div style={{ background: '#00201B', padding: '64px 24px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 24, flexWrap: 'wrap' }}>
          ...
          >
            {canPublishDirect ? 'Publicar aviso' : 'Solicitar publicación'} <IconChevronRight style={{ width: 16, height: 16 }} />
          </button>
        </div>
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          ...
          <div style={{ position: 'relative', background: '#fff', borderRadius: 20, width: '100%', maxWidth: 480, padding: '32px 28px', boxShadow: '0 24px 80px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
              <button onClick={() => setShowNew(false)} style={{ flex: 1, padding: '12px', fontSize: 14, fontWeight: 600, borderRadius: 10, border: '1px solid #E2E8F0', background: '#fff', color: '#64748B', cursor: 'pointer' }}>Cancelar</button>
              <button onClick={() => setShowNew(false)} style={{ flex: 2, padding: '12px', fontSize: 14, fontWeight: 700, borderRadius: 10, border: 'none', background: '#0D9488', color: '#fff', cursor: 'pointer' }}>
