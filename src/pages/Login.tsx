      ...
      <div className="hidden lg:flex flex-col justify-between w-[420px] shrink-0 bg-text p-12">
        ...
        <div className="flex gap-3 flex-wrap">
          {['150+ comunidades', '98% satisfacción', 'Soporte en español'].map(b => (
            <span key={b} className="text-xs text-white/50 border border-white/15 rounded-full px-3 py-1">{b}</span>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-white">
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-3.5 py-2.5 rounded-lg border text-sm text-text placeholder:text-muted/60 outline-none transition-colors focus:border-primary focus:ring-2 focus:ring-primary/20 ${errors.email ? 'border-alert-red bg-red-50' : 'border-border bg-white'}`}
                placeholder="nombre@correo.cl"
              />
              {errors.email && (
                <p id="email-error" role="alert" className="mt-1.5 text-xs text-alert-red flex items-center gap-1">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-3.5 py-2.5 rounded-lg border text-sm text-text placeholder:text-muted/60 outline-none transition-colors focus:border-primary focus:ring-2 focus:ring-primary/20 ${errors.password ? 'border-alert-red bg-red-50' : 'border-border bg-white'}`}
                placeholder="••••••••"
              />
              {errors.password && (
                <p id="password-error" role="alert" className="mt-1.5 text-xs text-alert-red flex items-center gap-1">
          ...
          <p className="mt-6 text-center text-sm text-muted">
            ¿Eres nuevo?{' '}
            <Link to="/crear-cuenta" className="text-primary font-semibold hover:text-accent transition-colors">
              Crear cuenta
            </Link>
          </p>

          <div className="mt-8 pt-6 border-t border-border">
            <Link to="/" className="text-xs text-muted hover:text-text transition-colors flex items-center justify-center gap-1">
